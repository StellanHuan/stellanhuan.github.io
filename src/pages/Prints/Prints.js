...
(async function(){
  ...
  document.getElementById('printsGrid').innerHTML=prints.map((p,i)=>`
    <div class="print-card" onclick="Lightbox.open([{file:'../../images/${p.photo}',title:p.title}])" style="animation:fadeUp .5s var(--ease-out) ${i*.1}s both">
      <img src="../../images/${p.photo}" alt="${p.title}" loading="lazy">
      <div class="print-info">
        <h3>${p.title}</h3>
        <div class="print-meta">
          <span>${p.size}</span><span>${p.paperType||''}</span><span>${p.edition||''}</span>
        </div>
        <div class="print-footer">
          <span class="print-price">¥${p.price}</span>
          <span class="print-size">库存 ${p.stock} 张</span>
        </div>
      </div>
    </div>`).join('');
